...
- **Next steps (docs & CI):** Scripts CI workflow (`.github/workflows/scripts-ci.yml`) runs when `scripts/` change: validates solution registry using the example. E2E: Support page without query params (Get help section). CONTRIBUTING § Full verification: optional scripts validation step; RELEASE-CHECKLIST: CI green note for scripts/ changes. NEXT-STEPS: E2E list (Support page tests), scripts-ci, Quick reference multi-instance; ANDROID-ROADMAP: multi-instance and Updates in Implemented. Web README: API instance-config, E2E coverage (Changelog, instance-config, Support page), Multi-instance instance ID in About. DEVELOPMENT-SETUP §5 Multi-instance provisioning; DESIGN-UI-AND-GAPS multi-instance in Data export.
- **Multi-instance (Phase 1):** Instance/solution ID and support for one-instance-per-farm deployments. **Web:** `NEXT_PUBLIC_SOLUTION_ID` and `NEXT_PUBLIC_SUPPORT_URL` (see .env.example); Settings → About shows "Instance: &lt;id&gt;" when set and Help & support / Suggest a feature / Report a problem links (with solutionId in URL); GET /api/instance-config returns { solutionId, supportBaseUrl }; GET /support?solutionId=...&topic=suggest|report minimal support page; sitemap includes /support. **Scripts (repo root):** create-solution.js, update-solution.js, env-for-solution.js, list-solutions.js, validate-registry.js; see docs/MULTI-INSTANCE-STRATEGY.md §5. **Android:** BuildConfig.SOLUTION_ID and SUPPORT_BASE_URL; Settings → About shows instance ID and support links when set. **OpenAPI:** instance-config path and InstanceConfigResponse schema; RELEASE-CHECKLIST and functions/README document deploy-to-all-instances (list-solutions.js --project-ids).

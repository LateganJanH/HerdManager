    ...
    <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8 space-y-8">
      ...
      >
        ...
        <div className="absolute inset-x-4 top-4 z-10 flex items-center justify-between gap-2">
          {farm != null && (
            <span
              className="rounded-lg bg-white/10 px-3 py-2 text-2xl font-bold tracking-tight text-white"
        ...
        <div className="relative z-10">
          ...
          <p className="mt-2 text-white/90 text-readable-lg">
      ...
      <section aria-label="Quick stats">
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {loading ? (
            ...
            <>
              <div className="rounded-card bg-white dark:bg-stone-800 border border-stone-200 dark:border-stone-600 p-5 shadow-card hover:shadow-card-hover transition-shadow">
              ...
              <div className="rounded-card bg-white dark:bg-stone-800 border border-stone-200 dark:border-stone-600 p-5 shadow-card hover:shadow-card-hover transition-shadow">
                ...
                <p className="mt-1.5 text-2xl font-semibold tracking-tight text-accent-alert">
                  {dueSoonCount > 0 ? dueSoonCount : "—"}
                </p>
                <p className="mt-2 text-sm text-stone-600 dark:text-stone-400">Calving, checks, withdrawal & weaning</p>
                {dueSoonCount > 0 && (
                  <>
                    {dueSoonPreview.length > 0 && (
                      <ul className="mt-3 space-y-1.5 text-sm text-stone-700 dark:text-stone-300" aria-label="Upcoming alerts preview">
              ...
              <div className="rounded-card bg-white dark:bg-stone-800 border border-stone-200 dark:border-stone-600 p-5 shadow-card hover:shadow-card-hover transition-shadow">
                ...
                <p className="mt-1.5 text-2xl font-semibold tracking-tight text-accent-success">
                  {stats.calvingsThisYear > 0 ? stats.calvingsThisYear : "—"}
                </p>
                <p className="mt-2 text-sm text-stone-600 dark:text-stone-400">This year</p>
              </div>
              <Link
      ...
      <section aria-labelledby="status-overview-heading">
        <h2 id="status-overview-heading" className="text-lg font-semibold tracking-tight text-stone-900 dark:text-stone-100">
          Status overview
        </h2>
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
        {loading ? (
          <>
            <div className="rounded-card bg-white dark:bg-stone-800 border border-stone-200 dark:border-stone-600 p-5 shadow-card animate-pulse" aria-hidden>
              <div className="h-6 w-24 rounded bg-stone-200 dark:bg-stone-600" />
              <div className="mt-3 space-y-2">
                {[1, 2, 3, 4].map((i) => (
                  <div key={i} className="flex justify-between">
                    <div className="h-4 w-20 rounded bg-stone-200 dark:bg-stone-600" />
                    <div className="h-4 w-8 rounded bg-stone-200 dark:bg-stone-600" />
                  </div>
                ))}
              </div>
            </div>
            <div className="rounded-card bg-white dark:bg-stone-800 border border-stone-200 dark:border-stone-600 p-5 shadow-card animate-pulse" aria-hidden>
              <div className="h-6 w-28 rounded bg-stone-200 dark:bg-stone-600" />
              <div className="mt-3 h-4 w-full max-w-[200px] rounded bg-stone-200 dark:bg-stone-600" />
              <div className="mt-2 h-4 w-32 rounded bg-stone-200 dark:bg-stone-600" />
            </div>
          </>
        ) : (
        ...
        <div className="rounded-card bg-white dark:bg-stone-800 border border-stone-200 dark:border-stone-600 p-5 shadow-card">
          ...
          {Object.keys(stats.byStatus).length > 0 ? (
            <ul className="mt-3 space-y-1.5 text-base text-stone-700 dark:text-stone-300">
              {Object.entries(stats.byStatus).map(([name, count]) => (
                <li key={name} className="flex justify-between gap-4">
                  <span>{name}</span>
                  <span className="font-medium tabular-nums text-stone-900 dark:text-stone-100">{count}</span>
                </li>
              ))}
            </ul>
          ) : (
        ...
        <div className="rounded-card bg-white dark:bg-stone-800 border border-stone-200 dark:border-stone-600 p-5 shadow-card">
          ...
          <ul className="mt-3 space-y-1.5 text-base text-stone-700 dark:text-stone-300">
            <li className="flex justify-between gap-4">
              <span>Calvings this year</span>
              <span className="font-medium tabular-nums text-stone-900 dark:text-stone-100">{stats.calvingsThisYear}</span>
            </li>
            <li className="flex justify-between gap-4">
              <span>Open / pregnant</span>
              <span className="font-medium tabular-nums text-stone-900 dark:text-stone-100">{stats.openPregnant}</span>
            </li>
          </ul>
        </div>
          </>
      ...
      <section className="rounded-card bg-white dark:bg-stone-800 border border-stone-200 dark:border-stone-600 p-5 shadow-card" aria-labelledby="analytics-heading">
        <h2 id="analytics-heading" className="text-lg font-semibold tracking-tight text-stone-900 dark:text-stone-100">
          Analytics
        </h2>
      ...
      <section
        ...
        aria-labelledby="connect-herd-heading"
      >
        <h2 id="connect-herd-heading" className="text-lg font-semibold tracking-tight text-stone-900 dark:text-stone-100">
          Connect your herd
        </h2>

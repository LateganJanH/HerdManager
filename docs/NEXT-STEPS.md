...
|------------|----------------------------------------------------------|
| **Web**    | ~~Firebase Auth + Firestore stats; linked devices; full animal detail; real-time listeners; withdrawal alerts (Alerts tab, Home due-soon); photo display when Storage URLs available~~ Done. |
| **Android**| ~~Herd list pull-to-refresh sync; empty-state CTA; loading/skeleton; offline/sync indicator; sync on app resume; withdrawal alerts (Alerts screen, Home and Herd list due-soon + banner)~~ Done. |
